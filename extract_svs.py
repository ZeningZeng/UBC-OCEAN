...
class SingleDICOMDataset(Dataset):
    ...
    def get_patch(self):
        ...
        for x, y in cors:
            ...
            if np.sum(mask_bg) < min(down_h, thu_height - y) * min(down_w, thu_width - x) * 0.7:
...
external_data_dir = '/home/data1/zzn/Projects/kaggle/wsi/PKG - PTRC-HGSOC'
transform = T.Compose([T.ToTensor(), T.Resize((224, 224), antialias=True), T.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])])
...
with torch.no_grad():
    for class_name in os.listdir(external_data_dir):
        ...
        for name in os.listdir(os.path.join(external_data_dir, class_name)):
            save_name = '92' + name[4:7]
            if save_name + '.pt' in os.listdir(feature_dir):
                continue
            start = time.time()
            ...
            torch.save(features, os.path.join(feature_dir, save_name + '.pt'))

            if int(save_name) not in list(train_csv['image_id']):
                train_csv.loc[len(train_csv)] = [int(save_name), class_name, dicomdataset.wsi.dimensions[0], dicomdataset.wsi.dimensions[1], False]

            dicomdataset.wsi.close()
            end = time.time()
            print('finish', save_name, class_name, len(dicomdataset), end - start)
